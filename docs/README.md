...
```
Added /cvmfs/dunedaq.opensciencegrid.org/tools/dbt/v6.0.4/bin -> PATH
Added /cvmfs/dunedaq.opensciencegrid.org/tools/dbt/v6.0.4/scripts -> PATH
...
Each time that you want to work with a DUNE DAQ work area in a fresh Linux shell, you'll need to set up daq-buildtools. While you could repeat the method above, if it's a pre-existing work area a better method is to `cd` into it and source the file named `env.sh`. Work areas are described momentarily. 
...
```
dbt-setup-release -b candidate <candidate release> 
```
...
Find a directory in which you want your work area to be a subdirectory (home directories are a popular choice) and `cd` into that directory. Then think of a good name for the work area (give it any name, but we'll refer to it as "MyTopDir" in this document). If you want to build against the nightly release (i.e., the official DUNE DAQ package installation which updates every night), run:
```sh
dbt-create [-i/--install-pyvenv] -n <nightly release> <name of work area subdirectory> # E.g., N22-04-18 or last_successful
