...
```
If you type `dbt-` followed by the `<tab>` key you'll see a listing of available commands, which include `dbt-create`, `dbt-build.py`, `dbt-setup-release` and `dbt-workarea-env`. These are all described in the following sections. 
...
```sh
dbt-create [-c/--clone-pyvenv] <release> <name of work area subdirectory> # dunedaq-v2.9.0 is the most recent frozen release as of Dec-16-2021
cd <name of work area subdirectory>
```

The option `-c/--clone-pyvenv` for `dbt-create` is optional. If used, the python virtual environment created in the work area will be a clone of an existing one from the release directory. This avoids the compilation/installation of python modules using the `pyvenv_requirements.txt` in the release directory, and speeds up the work-area creation significantly. The first time running `dbt-create` with this option on a node may take longer timer since cvmfs needs to fetch these files into local cache first.

The second step's important: remember to `cd` into the subdirectory you just created after `dbt-create` finishes running. 
...
Since the `dunedaq-v2.6.0` release, `dbt-create` has supported not only frozen releases but the use of nightly built releases as well. Add `-n` to the `dbt-create` command above to use nightly releases, e.g. `dbt-create -n N21-05-13 <your work area subdirectory>`. Pass `-l` to `dbt-create` in order to list all available frozen releases, and `-l -n` to list all available nightly releases.
