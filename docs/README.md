...
```
The commands include `dbt-create.sh`, `dbt-build.sh`, and `dbt-refresh-work-env`; these are all described in the following sections.
...
In order to access the applications, libraries and plugins built in your `./build` area during the above procedure, the system needs to be instructed on where to look for them. This is handled by again running `dbt-refresh-work-env`:
```
dbt-refresh-work-env
```

Note that in general if you add a new repo to your work area, after building your new code - and hence putting its output in `./build` - you'll need to run `dbt-refresh-work-env`. 
...
One of the packages that's part of the standard DUNE DAQ package suite which gets set up the first time you run `dbt-refresh-work-env` is cmdlib. This package comes with a basic implementation that is capable of sending available command objects from a pre-loaded file, by typing their command IDs to standard input. This command facility is useful for local, test oriented use-cases. In the same runtime area, launch the application like this:
```
