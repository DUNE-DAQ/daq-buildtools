...
function spack_setup_env() {
	...
	spack_setup_script=$( realpath $SPACK_RELEASES_DIR/$SPACK_RELEASE/spack-installation/share/spack/setup-env.sh )
	if [[ -z $spack_setup_script ]]; then   # Backwards compatibility with the old directory structure
	    spack_setup_script=$( realpath $SPACK_RELEASES_DIR/$SPACK_RELEASE/default/spack-installation )
	fi
